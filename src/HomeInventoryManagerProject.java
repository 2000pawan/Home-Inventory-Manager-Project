...
public class HomeInventory extends JFrame
{
    ...
    static homeinventory.HomeInventoryManagerProject[] myInventory = new homeinventory.HomeInventoryManagerProject[maximumEntries];
    int currentEntry;
...
// open file for entries
        try
        {
            ...
            if (numberEntries != 0)
            {
                ...
                {
                    myInventory[i] = new homeinventory.HomeInventoryManagerProject();
...
// move all entries below new value down one position unless at end
        if (currentEntry != numberEntries)
        {
            for (int i = numberEntries; i >= currentEntry + 1; i--)
            {
                ...
                myInventory[i - 2] = new homeinventory.HomeInventoryManagerProject();
            }
        }
        myInventory[currentEntry - 1] = new homeinventory.HomeInventoryManagerProject();
...
// move all entries under j up one level
            ...
            {
                myInventory[i - 1] = new homeinventory.HomeInventoryManagerProject();
...
//InventoryItem.java:
package HomeInventory;
public class HomeInventoryManagerProject extends JFrame
{
